    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>编辑备案信息</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="form-box">
...
<script>
    ...
    export default {
        ...
        methods: _.extend({},mapActions(['updateTable']),{
            onSubmit() {
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        this.updateTable(this.form).then(response => {
                            this.$message.success('更新备案信息成功！');
                            this.$router.push('/tableList');
                        }, error => {
                            this.$message.error('更新备案失败，请检查网络！');
                        })
                    } else {
                        return false;
                    }
        ...
        mounted() {
            ...
            if(!this.$store.getters.getTableById(tableId)){
                this.$message.error('备案信息不存在，请核对后重试');
                this.$router.push('/tableList');
            }else{
                this.form = this.$store.getters.getTableById(tableId);
                try
                {
                    this.form.expiredDate = new Date(this.form.expiredDate);
                }
                catch(err)
                {
                }
